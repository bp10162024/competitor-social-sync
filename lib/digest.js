...
const MODEL = process.env.COMPETITOR_SOCIAL_MODEL || "claude-sonnet-5";
...
async function slackJoin() {
  // best-effort: works for public channels; private channels must invite the bot manually
  try {
    ...
    if (!j.ok) console.log("[digest] conversations.join:", j.error);
  } catch (e) {
    console.log("[digest] conversations.join threw:", e.message);
  }
}
...
function platformSection(platform, snaps, priorMap) {
  ...
  for (const s of rows) {
    ...
    if (platform === "linkedin") {
      ...
      // YouTube scraper doesn't return reliable per-video likes, so engagement is meaningless here — show library size
      tail = s.post_count != null ? ` · ${fmtNum(s.post_count)} videos` : "";
    } else if (s.engagement_rate != null) {
      // a viral post can push avg likes far past follower count (e.g. 7shifts reels) — flag rather than print >100%
      const er = Number(s.engagement_rate);
      tail = er > 100 ? ` · :rocket: viral reach` : ` · eng ${er.toFixed(2)}%`;
    }
